...
Selector.initHtml = function(ele){
	...
	if(len>0||ele.innerHTML.trim()==""){
		...
		arr.push("</div>");
		var html = arr.join('');
		if(html.indexOf("_NSWT_NOID_")>0){
			html = html.replace("_NSWT_NOID_",id);
		}
		ele.innerHTML = html;
	}else{
		ele.textField = ele.children[0];
		...
		if(html.indexOf("_NSWT_NOID_")>0){
			ele.innerHTML = html.replace("_NSWT_NOID_",id);
		}
	}
...
Selector.initMethod  = function(ele){
	...
	if(!ele.$A("listurl")){//自定义页面不需要如下方法
		...
		ele.clear = function(){
			...
			if(!this.$T("DIV")[0].children[0]){
				return;//已经是空的了
			}
			this.$T("DIV")[0].children[0].innerHTML = "";
			this.Items = this.$T("DIV")[0].children[0].$T("a");
			this.value = "";
			this.textField.value = "";
		...
		ele.remove = function(index){
			...
			this.options = this.Items = this.$T("DIV")[0].children[0].$T("a");
		...
		ele.addBatch = function(arr,index){
			...
			if(!this.Items||this.Items.length==0){
				this.$T("DIV")[0].children[0].innerHTML = html.join('\n');
				this.options = this.Items = this.$T("DIV")[0].children[0].$T("a");
				return;
			}
			...
			if(this.Items.length>10){
				this.$T("DIV")[0].children[0].style.height = "15em";
			}
		}
...
Selector.loadData = function(ele,func){
	...
	Server.sendRequest("com.nswt.framework.controls.CodeSourcePage.getData",ele.Params,function(response){
		...
		if(dt){
			...
			ele.$T("DIV")[0].children[0].innerHTML = html.join('\n');
			ele.options = ele.Items = ele.$T("DIV")[0].children[0].$T("a");
			if(ele.Items.length>10){
				ele.$T("DIV")[0].children[0].style.height = "15em";
			}
		}
