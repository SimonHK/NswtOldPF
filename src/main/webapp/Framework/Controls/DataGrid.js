...
Constant.SortString = "_NSWT_SORTSTRING";
Constant.PageIndex = "_NSWT_PAGEINDEX";
Constant.PageTotal = "_NSWT_PAGETOTAL";
Constant.TagBody = "_NSWT_TAGBODY";
Constant.ID = "_NSWT_ID";
Constant.Method = "_NSWT_METHOD";
Constant.Page = "_NSWT_PAGE";
Constant.Size = "_NSWT_SIZE";
Constant.Null = "_NSWT_NULL";
Constant.InsertRow = "_NSWT_INSERTROW";
Constant.DataTable = "_NSWT_DataTable";
...
DataGrid.toExcelSubmit = function(ele,toExcelPageFlag,columnIndexes,columnWidths,rows){
	...
	var input = $(xls+"_NSWT_ToExcelPageFlag");
	if(!input){
		input  = document.createElement("input");
		
	}
	input.type = "hidden";
	input.id =xls+"_NSWT_ToExcelPageFlag";
	input.name = xls+"_NSWT_ToExcelPageFlag";
	input.value= toExcelPageFlag? "1":"0";
	f.appendChild(input);

	input = $(xls+"_NSWT_Widths");
	if(!input){
		input  = document.createElement("input");
	}
	input.type="hidden";
	input.id =xls+"_NSWT_Widths";
	input.name = xls+"_NSWT_Widths";
	input.value = columnWidths.join();
	f.appendChild(input);
	
	input = $(xls+"_NSWT_Indexes");
	if(!input){
		input  = document.createElement("input");
	}
	input.type="hidden";
	input.id =xls+"_NSWT_Indexes";
	input.name = xls+"_NSWT_Indexes";
	input.value = columnIndexes.join();
	f.appendChild(input);
	
	if(rows){
		input = $(xls+"_NSWT_Rows");
		if(!input){
			input  = document.createElement("input");
		}
		input.type="hidden";
		input.id =xls+"_NSWT_Rows";
		input.name = xls+"_NSWT_Rows";
...
DataGrid.loadData = function(ele,func,dc){
	...
	Server.sendRequest("com.nswt.framework.controls.DataGridPage.doWork",dc,function(response){
		try{
			...
			if(DataGrid.getParam(id,"_NSWT_SCROLL")=="true"){
