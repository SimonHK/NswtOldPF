...
function addSave(){
	...
	Server.sendRequest("com.nswt.cms.site.Link.add",dc,function(response){
...
function editSave(){
	...
	Server.sendRequest("com.nswt.cms.site.Link.edit",dc,function(response){
...
function save(){
	DataGrid.save("dg2","com.nswt.cms.site.Link.save",function(){DataGrid.loadData('dg2');});
}
...
function del(){
	...
	Dialog.confirm("您确认要删除吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.nswt.cms.site.Link.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData('dg2');
				}
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.nswt.cms.site.Link.sortColumn",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg2");
			}
...
%>
	<table width="100%" border="0" cellspacing="0" cellpadding="6">
		...
		<tr>
			...
			<z:datagrid id="dg2" method="com.nswt.cms.site.Link.dg1DataBind">
