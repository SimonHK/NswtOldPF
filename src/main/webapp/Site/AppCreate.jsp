...
<head>
    ...
    <script>
        ...
        function add(){
            ...
            diag.MessageTitle = "新建业务";
            diag.Message = "新建一个业务，并设置业务的名称、模板文件、工作流和其他属性";
        ...
        function del(){
            ...
            Dialog.confirm('警告：您确认要删除"'+$V("Name")+'"这个业务吗？\n这样会把业务下的所有相关文章都删除掉！',function(){
                dc.add("CatalogID",$V("CatalogID"));
                Server.sendRequest("com.nswt.cms.site.Catalog.del",dc,function(response){
                    if(response.Status==0){
                        Dialog.alert(response.Message);
                    }else{
                        Dialog.alert("删除业务成功",function(){
                            Tree.loadData("tree1");
                            $S("CatalogID","");
                            onTabChange("Basic");
