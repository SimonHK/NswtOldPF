  ...
  var content = contents.join(PAGE_SPLIT);
	...
	Server.sendRequest("com.nswt.cms.document.Article.checkBadWord",dc,function(response){
		if(response.Status == 1){
			...
			if (isBadWordExist) {
				Dialog.wait("正在处理...");
				Server.sendRequest("com.nswt.cms.document.Article.save",dc,function(response){
					Dialog.closeEx();
					if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("确认删除吗？点击”确认“删除，点击”取消“返回。",function(){
		...
		Server.sendRequest("com.nswt.cms.document.Article.del",dc,function(response){
			if(response.Status==1){
				Dialog.alert("文章删除成功");
		    	window.close();
			}else{
		...
		if(!$("CommentFlag").checked){
	  	...
	  	Server.sendRequest("com.nswt.cms.document.Article.checkBadWord",dc,function(response){
			if(response.Status == 1){
				...
				if (isBadWordExist) {
					Dialog.wait("正在发布...");
					Server.sendRequest("com.nswt.cms.document.Article.saveAndPublish",dc,function(response){
						Dialog.closeEx();
						if(response.Status==1){
							var st = response.get("SaveTime");
							$("SaveTime").innerHTML = st;
