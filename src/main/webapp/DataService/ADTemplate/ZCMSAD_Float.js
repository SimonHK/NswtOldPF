...
function showADContent() {
  ...
  var str = "<div id='nswtpAD_"+this.PosID+"' style='left:"+this.PaddingLeft+"px;top:"+this.PaddingTop+"px;width:"+this.Width+"px; height:"+this.Height+"px; position: absolute;visibility: visible;z-index:999999;' onMouseOver='"+this.ID+"_pause_resume();' onMouseOut='"+this.ID+"_pause_resume();'>";
  var AD = eval('('+content+')');
  ...
  str += "<div style='text-align:right;'><a href='#;' onclick='javascript:document.getElementById(\"nswtpAD_"+this.PosID+"\").style.display=\"none\"'>¹Ø±Õ</a></div>";
  str += "</div>";
  document.write(str);
}
...
function changePos(float) {	
	...
	document.getElementById("nswtpAD_"+float.PosID).style.left = (float.PaddingLeft + (Math.max(document.documentElement.scrollLeft, document.body.scrollLeft)))+"px";
	document.getElementById("nswtpAD_"+float.PosID).style.top = (float.PaddingTop + (Math.max(document.documentElement.scrollTop, document.body.scrollTop)))+"px";
	if (float.Yon){
