...
function save(){
	...
	Server.sendRequest("com.nswt.platform.RoleTabCatalog.dg1Edit",dc,function(response){
		$("AllSelect").checked =false;
...
function clickCheckBox(ele){
	...
	for(var i=1;i<length;i++){
		if(tr==trs[i]){
			...
			//当选中文章栏目的文章审核权限时，自动选中该栏目的栏目浏览、文章管理权限，栏目管理权限不作处理
			...
			for(var j=i+1;j<length;j++){
				...
				//当选中文章栏目的文章审核权限时，自动选中该栏目的子栏目的栏目浏览、文章管理权限，栏目管理权限不作处理
				if(flag && "article_audit"==ele.id){
			...
			//当选中每一行的第一个权限时，自动选中所有上级栏目的第一个权限
			<%
			if(map.size()>0){
			%>
...
<tr>
<z:init method="com.nswt.platform.RoleTabCatalog.init">
...
<z:datagrid id="dg1" method="com.nswt.platform.RoleTabCatalog.dg1DataBind" page="false" autoFill="false">
